...
void prompt()
{
    int flag;
    int run;
    int dir_size=1,home_dirsz;
    char *h_name, u_name;
    long unsigned int lim = 10000;
    char *comm;
    char **sep_comm;
    char relative[10000];
    char curdir_name[10000], home_dir[10000];
    // get user name
    ...
    while (run)
    {
        ...
        else
        {
            ...
            if (strlen(curdir_name) > home_dirsz)
            {
                printf("~");
                for(int i =home_dirsz;i < strlen(curdir_name)-1;i++)
